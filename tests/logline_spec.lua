...
describe('logline.log', function()
  ...
  it('leaves no visible marker on the line', function()
    local bufnr = open({ 'const subtotal = 1' }, 'typescript', 1, 6)
    logline.log('log')
    assert.is_falsy(lines_of(bufnr)[2]:find('logline', 1, true))
    assert.is_falsy(lines_of(bufnr)[2]:find('//', 1, true))
  end)

  it('is still recognisable as its own statement', function()
    local bufnr = open({ 'const subtotal = 1' }, 'typescript', 1, 6)
    logline.log('log')
    assert.is_true(require('logline.statement').is_logline(lines_of(bufnr)[2], ''))
  end)

  it('does not mistake a hand-written console call for its own', function()
    assert.is_false(require('logline.statement').is_logline("console.log('hello there')", ''))
    assert.is_false(require('logline.statement').is_logline("console.log('a:', a)", ''))
  end)
